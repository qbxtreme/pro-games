(function () {
  ...
  function catchMultiplier() {
    let mult = 1.2 + (C.captureRateBonus || 0);
    GEAR.forEach((e) => {
  ...
  function attackPower() {
    ...
    let atk = (type?.atk || 8) + state.level * 3 + 4;
    GEAR.forEach((u) => {
  ...
  function updateActionButtons() {
    ...
    setBtnVisible(document.getElementById("fight-btn"), btnSticky.mob && !battle && !zone.isPier && !zone.isPvp);
  ...
  function feedFish() {
    ...
    if (state.coins < FEED_COST) {
      showToast(ui("feedNeedCoins", { cost: FEED_COST }));
      return;
    }
    state.coins -= FEED_COST;
    addExp(FEED_EXP);
    showToast(ui("feedSuccess", { exp: FEED_EXP }));
    updateHud();
    saveState();
  }
  ...
  function setupJoystick() {
    ...
    base.addEventListener("pointermove", (e) => {
      ...
      let dx = e.clientX - cx;
      let dy = e.clientY - cy;
      const max = Math.min(56, r.width * 0.42);
