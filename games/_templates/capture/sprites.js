(function () {
  ...
  function drawWorldTile(ctx, sx, sy, tw, th, zone, seed) {
    ...
    if (zone.decor === "park" || zone.decor === "pier") {
      ...
      ctx.fillStyle = shade(base, zone.decor === "pier" ? 0.15 : 0.2);
      if ((seed % 3) === 0) ctx.fillRect(sx + (tw - pathW) / 2, sy, pathW, th);
      else if ((seed % 3) === 1) ctx.fillRect(sx, sy + (th - pathW) / 2, tw, pathW);
    }

    if (zone.decor === "ocean" || zone.decor === "depths") {
      if ((seed % 5) === 0) {
        ctx.fillStyle = "rgba(255,255,255,0.08)";
        ctx.beginPath();
        ctx.ellipse(sx + tw * 0.5, sy + th * 0.6, tw * 0.2, 4, 0, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    ...
    if (zone.decor === "depths" && (seed % 9) === 0) {
      ...
      ctx.beginPath();
      ctx.arc(sx + tw * 0.5, sy + th * 0.5, 6, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  function drawMobBody(ctx, c, dark, rx, ry, animT, opts) {
    opts = opts || {};
    ...
    const tailSwing = Math.sin(animT * 6) * 0.15;
    ctx.strokeStyle = dark;
    ctx.lineWidth = 3;
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(-rx * 0.72, ry * 0.05);
    ctx.quadraticCurveTo(-rx * 0.95, ry * (0.2 + tailSwing), -rx * 0.78, ry * 0.35);
    ctx.quadraticCurveTo(-rx * 0.95, ry * (-0.1 + tailSwing), -rx * 0.72, ry * (-0.05 + tailSwing * 0.5));
    ctx.stroke();
    ctx.fillStyle = c;
    ctx.beginPath();
    ctx.moveTo(-rx * 0.72, ry * 0.05);
    ctx.lineTo(-rx * 0.98, ry * 0.22);
    ctx.lineTo(-rx * 0.98, ry * -0.12);
    ctx.closePath();
    ctx.fill();
  }
  ...
  function drawWildMob(ctx, x, y, type, animT, mobLevel, opts) {
    opts = opts || {};
    ...
    drawMobBody(ctx, main, dark, rx, ry, animT, { mouthOpen: !!type.rare });
  ...
  function drawMythicMob(ctx, x, y, animT, zone, mobLevel) {
    ...
    drawMobBody(ctx, c, dark, 44, 36, animT, { mouthOpen: true });
    ...
    ctx.strokeStyle = "#111";
    ctx.lineWidth = 3;
    ctx.strokeText(`MYTHIC Lv ${mobLevel}`, 0, 58);
    ctx.fillText(`MYTHIC Lv ${mobLevel}`, 0, 58);
    ctx.restore();
  }
  ...
  function drawTrainer(ctx, x, y, scale, style, facing, animT, opts) {
    ...
    if (opts.riding && opts.dinoType) {
      ...
      drawMobBody(ctx, main, dark, 22, 18, animT, { eyeGlow: false, plates: true });
      ...
      if (!opts.hideCompanion) {
        ...
        drawMobBody(ctx, main, dark, 10, 8, animT, { eyeGlow: false, plates: false });
        ctx.translate(18, -6);
      }
