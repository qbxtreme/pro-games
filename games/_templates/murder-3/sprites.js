(function () {
  ...
  function drawFloorTile(ctx, c, r, room, animT) {
    const x = c * MMMap.TILE;
    const y = r * MMMap.TILE;
    const T = MMMap.TILE;
    ...
    const nearWall = MMMap.tiles[r * MMMap.COLS + c] === 0
      || (c > 0 && MMMap.tiles[r * MMMap.COLS + c - 1] === 0)
      || (c < MMMap.COLS - 1 && MMMap.tiles[r * MMMap.COLS + c + 1] === 0)
      || (r > 0 && MMMap.tiles[(r - 1) * MMMap.COLS + c] === 0)
      || (r < MMMap.ROWS - 1 && MMMap.tiles[(r + 1) * MMMap.COLS + c] === 0);
    if (nearWall) {
  ...
  function drawWallTile(ctx, c, r) {
    if (MMMap.isExteriorWall && MMMap.isExteriorWall(c, r)) {
  ...
  function drawRoomLabel(ctx, room) {
    const cx = ((room.c0 + room.c1) / 2 + 0.5) * MMMap.TILE;
    const ty = room.r0 * MMMap.TILE + 14;
  ...
  function drawProp(ctx, p, animT) {
    animT = animT || 0;
    const x = p.c * MMMap.TILE;
    const y = p.r * MMMap.TILE;
    const T = MMMap.TILE;
    const cx = x + T / 2;
    const cy = y + T - 6;
  ...
  function drawPuzzleSpot(ctx, s, animT, done) {
    ...
    ring.addColorStop(0, done ? "rgba(76,175,80,0.35)" : `rgba(66,165,245,${pulse * 0.35})`);
    ring.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = ring;
    ctx.beginPath();
    ctx.arc(0, 0, 22, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = done ? "rgba(56,142,60,0.9)" : `rgba(33,150,243,${0.55 + pulse * 0.3})`;
    ctx.lineWidth = 2.5;
    ctx.beginPath();
  ...
  function drawEscapeGate(ctx, gate, animT, active) {
    if (!gate) return;
    const pulse = 0.5 + Math.sin(animT * 0.09) * 0.4;
    ctx.save();
    ctx.translate(gate.x, gate.y);
    drawSoftShadow(ctx, 0, 14, 28, 10, 0.3);
    const ring = ctx.createRadialGradient(0, 0, 8, 0, 0, 32);
    ring.addColorStop(0, active ? `rgba(76,175,80,${0.3 * pulse})` : "rgba(120,120,120,0.15)");
    ring.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = ring;
    ctx.beginPath();
    ctx.arc(0, 0, 30, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = active ? `rgba(56,142,60,${0.85 * pulse})` : "rgba(158,158,158,0.5)";
    ctx.lineWidth = 2.5;
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText(active ? gate.emoji : "🔒", 0, 1);
    ctx.restore();
  }
  ...
  function drawRoomCeilingLight(ctx, room, animT, isNight) {
    const cx = ((room.c0 + room.c1) / 2 + 0.5) * MMMap.TILE;
    const cy = ((room.r0 + room.r1) / 2 + 0.5) * MMMap.TILE;
    const rw = (room.c1 - room.c0 + 1) * MMMap.TILE;
    const rh = (room.r1 - room.r0 + 1) * MMMap.TILE;
    ...
    if (isNight) {
      ...
      ctx.fillStyle = pool;
      ctx.fillRect(room.c0 * MMMap.TILE, room.r0 * MMMap.TILE, rw, rh);
    } else {
    ...
    const lx = cx;
    const ly = room.r0 * MMMap.TILE + 12;
  ...
  function visibleTileRange(cam, w, h) {
    const pad = 2;
    const minCol = Math.max(0, Math.floor((cam.x - w / 2) / MMMap.TILE) - pad);
    const maxCol = Math.min(MMMap.COLS - 1, Math.ceil((cam.x + w / 2) / MMMap.TILE) + pad);
    const minRow = Math.max(0, Math.floor((cam.y - h / 2) / MMMap.TILE) - pad);
    const maxRow = Math.min(MMMap.ROWS - 1, Math.ceil((cam.y + h / 2) / MMMap.TILE) + pad);
  ...
  function drawWorld(ctx, cam, w, h, animT, taskFlags, bodies, players, remotePlayers, playerColors, opts) {
    ...
    for (let r = vis.minRow; r <= vis.maxRow; r++) {
      for (let c = vis.minCol; c <= vis.maxCol; c++) {
        const t = MMMap.tiles[r * MMMap.COLS + c];
        if (t >= 1) {
          const room = MMMap.ROOM_DEFS.find((rm) => c >= rm.c0 && c <= rm.c1 && r >= rm.r0 && r <= rm.r1);
    ...
    MMMap.ROOM_DEFS.forEach((room) => {
      if (room.accentGlow) {
        const lx = (room.c0 + (room.c1 - room.c0) / 2 + 0.5) * MMMap.TILE;
        const ly = (room.r0 + (room.r1 - room.r0) / 2 + 0.5) * MMMap.TILE;
        const ag = ctx.createRadialGradient(lx, ly, 20, lx, ly, MMMap.TILE * 3.2);
        ...
        ctx.fillRect(
          room.c0 * MMMap.TILE,
          room.r0 * MMMap.TILE,
          (room.c1 - room.c0 + 1) * MMMap.TILE,
          (room.r1 - room.r0 + 1) * MMMap.TILE
        );
      }
    ...
    const donePuzzles = opts.donePuzzles || taskFlags || {};
    MMMap.getPuzzleStations().forEach((s) => drawPuzzleSpot(ctx, s, animT, !!donePuzzles[s.id]));
    drawEscapeGate(ctx, MMMap.getEscapeGate(), animT, !!opts.escapeUnlocked);
    ...
    if (isNight) {
      ...
      ctx.fillRect(-200, -200, MMMap.WORLD_W + 400, MMMap.WORLD_H + 400);
      ...
      MMMap.ROOM_DEFS.forEach((room) => {
        const lx = ((room.c0 + room.c1) / 2 + 0.5) * MMMap.TILE;
        const ly = room.r0 * MMMap.TILE + 20;
        const spill = ctx.createRadialGradient(lx, ly, 5, lx, ly, MMMap.TILE * 2);
        spill.addColorStop(0, "rgba(255,236,179,0.08)");
        spill.addColorStop(1, "rgba(0,0,0,0)");
        ctx.fillStyle = spill;
        ctx.fillRect(room.c0 * MMMap.TILE, room.r0 * MMMap.TILE, (room.c1 - room.c0 + 1) * MMMap.TILE, MMMap.TILE * 2);
      });
    } else {
      const dayWash = ctx.createLinearGradient(0, -200, MMMap.WORLD_W, MMMap.WORLD_H);
