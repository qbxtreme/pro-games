(function () {
  ...
  const SAVE_KEY = "brawlStarsMod";
  const COINS_KEY = "brawlStarsModCoins";
  const BLING_KEY = "brawlStarsModBling";
  const ENERGY_BLASTS_KEY = "brawlStarsModEnergyBlasts";
  const LEVEL_KEY = "brawlStarsModLevel";
  const MAX_PLAYER_LEVEL = 100;
  const LEVEL_BASE_COST = 20;
  const LEVEL_COST_STEP = 100;
  const WIN_BLING_REWARD = 100;
  const WIN_ENERGY_REWARD = 100;
  const QUEST_KEY = "brawlStarsModQuest";
  const QUEST_PROGRESS_KEY = "brawlStarsModQuestProgress";
  const QUEST_CLAIMED_KEY = "brawlStarsModQuestClaimed";
  const PASS_TIER_KEY = "brawlStarsModPassTier";
  const PASS_XP_KEY = "brawlStarsModPassXp";
  ...
  const CHAR_PROG_KEY = "brawlStarsModCharProg";
  ...
  const MEGA_QUESTS = [
    { id: "mega-flawless", text: "Win without ever touching poison", reward: 200 },
    { id: "mega-slaughter", text: "Get 8 knockouts in one match", reward: 250 },
    { id: "mega-tank", text: "Win with 75% HP or more left", reward: 180 },
    { id: "mega-duo-champ", text: "Win a Duo Showdown match", reward: 220 },
    { id: "mega-blitz", text: "Win with 4+ kills before poison shrinks halfway", reward: 300 },
  ];
  ...
  const BRAWLER_UNLOCK_PREFIX = "brawlStarsModUnlock_";
  const SIRIUS_UNLOCK_KEY = "brawlStarsModSiriusUnlocked";
  const GOD_UNLOCK_KEY = "brawlStarsModGodUnlocked";
  const OOHLALA_UNLOCK_KEY = "brawlStarsModOohLaLaUnlocked";
  const COCO_UNLOCK_KEY = "brawlStarsModCocoUnlocked";
  ...
  const GAME_MODE_KEY = "brawlStarsModGameMode";
  const BRAWLER_SELECT_KEY = "brawlStarsModSelectedBrawler";
  const BS_GAME_MODES = [
    ...
    { id: "gem-grab", name: "Gem Grab", emoji: "💎", cat: "3v3", desc: "Collect 10 gems before the enemy team.", match: "solo", toast: "Gem Grab (Modded) — knockouts drop gems!", win: "Gem Grab victory!" },
    { id: "brawl-ball", name: "Brawl Ball", emoji: "⚽", cat: "3v3", desc: "Score 2 goals in the enemy net.", match: "solo", toast: "Brawl Ball (Modded) — shoot to score!", win: "GOOOAL! Brawl Ball win!" },
    { id: "goal-rush", name: "Goal Rush", emoji: "🥅", cat: "3v3", desc: "Fast soccer — first to score wins.", match: "solo", toast: "Goal Rush (Modded) — rush the goal!", win: "Goal Rush champions!" },
    { id: "basket-brawl", name: "Basket Brawl", emoji: "🏀", cat: "3v3", desc: "Sink baskets to win the match.", match: "solo", toast: "Basket Brawl (Modded) — nothing but net!", win: "Basket Brawl win!" },
    { id: "volley-brawl", name: "Volley Brawl", emoji: "🏐", cat: "3v3", desc: "Volleyball brawl — spike to score.", match: "solo", toast: "Volley Brawl (Modded) — set, spike, win!", win: "Volley Brawl victory!" },
    { id: "heist", name: "Heist", emoji: "🔐", cat: "3v3", desc: "Break the enemy safe before they break yours.", match: "solo", toast: "Heist (Modded) — crack the safe!", win: "Safe destroyed — Heist win!" },
    { id: "hot-zone", name: "Hot Zone", emoji: "🔥", cat: "3v3", desc: "Hold capture zones to fill the bar.", match: "solo", toast: "Hot Zone (Modded) — control the zone!", win: "Hot Zone captured!" },
    { id: "knockout", name: "Knockout", emoji: "🎯", cat: "3v3", desc: "Best of 3 rounds — no respawns.", match: "solo", toast: "Knockout (Modded) — one life only!", win: "Knockout round won!" },
    { id: "bounty", name: "Bounty", emoji: "⭐", cat: "3v3", desc: "Collect stars from knockouts.", match: "solo", toast: "Bounty (Modded) — stars for every KO!", win: "Bounty stars collected!" },
    { id: "wipeout", name: "Wipeout", emoji: "💥", cat: "3v3", desc: "First team to 8 knockouts wins.", match: "solo", toast: "Wipeout (Modded) — rack up KOs!", win: "Wipeout — 8 KOs!" },
    { id: "payload", name: "Payload", emoji: "🚂", cat: "3v3", desc: "Escort the cart to the finish line.", match: "solo", toast: "Payload (Modded) — push the cart!", win: "Payload delivered!" },
    { id: "trophy-thieves", name: "Trophy Thieves", emoji: "🏆", cat: "Special", desc: "Steal trophies and bank them safely.", match: "solo", toast: "Trophy Thieves (Modded) — grab and bank!", win: "Trophies secured!" },
    { id: "hold-the-trophy", name: "Hold the Trophy", emoji: "🥇", cat: "Special", desc: "Hold the trophy — don't get knocked out!", match: "solo", toast: "Hold the Trophy (Modded) — keep it safe!", win: "You held the trophy!" },
    { id: "present-plunder", name: "Present Plunder", emoji: "🎁", cat: "Special", desc: "Collect presents for your team.", match: "solo", toast: "Present Plunder (Modded) — grab the gifts!", win: "Presents plundered!" },
    { id: "big-game", name: "Big Game", emoji: "👹", cat: "Co-op PvE", desc: "Hunt the giant boss brawler.", match: "solo", toast: "Big Game (Modded) — take down the boss!", win: "Boss defeated!" },
    { id: "robo-rumble", name: "Robo Rumble", emoji: "🤖", cat: "Co-op PvE", desc: "Survive endless robo waves.", match: "solo", toast: "Robo Rumble (Modded) — survive the bots!", win: "Robo Rumble survived!" },
    { id: "boss-fight", name: "Boss Fight", emoji: "🐉", cat: "Co-op PvE", desc: "Team up to beat the mega boss.", match: "solo", toast: "Boss Fight (Modded) — defeat the boss!", win: "Boss Fight cleared!" },
    { id: "super-city-rampage", name: "Super City Rampage", emoji: "🏙️", cat: "Co-op PvE", desc: "Classic co-op — smash the city boss.", match: "solo", toast: "Super City Rampage (Modded)!", win: "City saved!" },
  ];
  const BS_GAME_CATEGORIES = ["Showdown", "3v3", "Special", "Co-op PvE"];
  ...
  function buildIslandsLayout(rnd) {
    const islands = 3 + Math.floor(rnd() * 3);
    for (let n = 0; n < islands; n++) {
      const ix = 520 + rnd() * (ARENA_W - 1040);
      const iy = 520 + rnd() * (ARENA_H - 1040);
      for (let i = 0; i < 2; i++) {
        const rw = 130 + rnd() * 90;
        const rh = 80 + rnd() * 50;
        addWall(ix + (rnd() - 0.5) * 120, iy + (rnd() - 0.5) * 90, rw, rh, "rock");
      }
      for (let i = 0; i < 4; i++) {
        const bw = 90 + rnd() * 25;
        const bh = 78 + rnd() * 18;
        addWall(ix + (rnd() - 0.5) * 160, iy + (rnd() - 0.5) * 130, bw, bh, "bush");
      }
    }
  }

  function buildCanyonLayout(rnd) {
    const gaps = 2 + Math.floor(rnd() * 2);
    for (let lane = 0; lane < gaps + 1; lane++) {
      const x = 520 + lane * ((ARENA_W - 1040) / gaps) + (rnd() - 0.5) * 80;
      const segments = 3 + Math.floor(rnd() * 2);
      for (let s = 0; s < segments; s++) {
        const rw = 70 + rnd() * 40;
        const rh = 180 + rnd() * 220;
        const y = 420 + s * ((ARENA_H - 840) / segments) + rnd() * 80;
        addWall(x, y, rw, rh, "rock");
      }
    }
    for (let i = 0; i < 14; i++) tryRandomWall("bush", rnd, 88, 112, 72, 95, 24);
  }
  ...
  function buildCrossroadsLayout(rnd) {
    const thick = 70 + rnd() * 30;
    addWall(CENTER_X - thick * 0.5, ARENA_H * 0.18, thick, ARENA_H * 0.64, "rock");
    addWall(ARENA_W * 0.18, CENTER_Y - thick * 0.5, ARENA_W * 0.64, thick, "rock");
    for (let i = 0; i < 4; i++) {
      const ang = (i / 4) * Math.PI * 2 + rnd() * 0.2;
      const dist = Math.min(ARENA_W, ARENA_H) * (0.22 + rnd() * 0.08);
      addWall(CENTER_X + Math.cos(ang) * dist, CENTER_Y + Math.sin(ang) * dist, 100 + rnd() * 60, 80 + rnd() * 40, "bush");
    }
    for (let i = 0; i < 6; i++) tryRandomWall("rock", rnd, 90, 160, 65, 100, 30);
  }

  function buildPitLayout(rnd) {
    const inner = Math.min(ARENA_W, ARENA_H) * (0.14 + rnd() * 0.06);
    const outer = Math.min(ARENA_W, ARENA_H) * (0.36 + rnd() * 0.08);
    for (let i = 0; i < 14; i++) {
      const ang = (i / 14) * Math.PI * 2;
      const rw = 110 + rnd() * 70;
      const rh = 70 + rnd() * 40;
      const x = CENTER_X + Math.cos(ang) * outer - rw * 0.5;
      const y = CENTER_Y + Math.sin(ang) * outer - rh * 0.5;
      addWall(x, y, rw, rh, "rock");
    }
    addWall(CENTER_X - inner, CENTER_Y - inner, inner * 2, inner * 2, "bush");
    for (let i = 0; i < 8; i++) tryRandomWall("bush", rnd, 88, 112, 72, 95, 24);
  }

  function buildGridLayout(rnd) {
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        const rw = 100 + rnd() * 50;
        const rh = 75 + rnd() * 35;
        const x = ARENA_W * 0.2 + col * ((ARENA_W * 0.6) / Math.max(1, cols - 1)) - rw * 0.5;
        const y = ARENA_H * 0.2 + row * ((ARENA_H * 0.6) / Math.max(1, rows - 1)) - rh * 0.5;
        addWall(x, y, rw, rh, rnd() > 0.5 ? "rock" : "bush");
      }
    }
  }
  ...
  function buildLayout(presetId, rnd) {
    ...
    else if (presetId === "islands") buildIslandsLayout(rnd);
    else if (presetId === "canyon") buildCanyonLayout(rnd);
    else if (presetId === "fortress") buildFortressLayout(rnd);
    else if (presetId === "crossroads") buildCrossroadsLayout(rnd);
    else if (presetId === "pit") buildPitLayout(rnd);
    else if (presetId === "grid") buildGridLayout(rnd);
  ...
  function migrateLegacyUnlocks() {
    const legacy = {
      brawlStarsModKitUnlocked: "kit",
      brawlStarsModLeonUnlocked: "leon",
      brawlStarsModSurgeUnlocked: "surge",
      brawlStarsModStarNovaUnlocked: "starnova",
      brawlStarsModGlowyUnlocked: "glowy",
      brawlStarsModDynamikeUnlocked: "dynamike",
    };
    try {
  ...
  function drawPoisonZone(useCam) {
    ...
    const safe = ctx.createRadialGradient(mx, my, poisonR * 0.5, mx, my, poisonR * pulse);
    safe.addColorStop(0, "rgba(0,0,0,1)");
    safe.addColorStop(0.82, "rgba(0,0,0,0.9)");
    safe.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = safe;
    ctx.beginPath();
    ctx.arc(mx, my, poisonR * pulse, 0, Math.PI * 2);
    ctx.fill();
  ...
  function hasOnlineTeammate() {
    return isTeamMode() && remotePlayers.length > 0;
  }
    ...
    if (isGodBossMode()) {
      ...
      if (!isGodBrawler(brawlerKey)) {
        [
          ...
          { x: CENTER_X + 320, kind: "surge" },
        ].forEach((s, i) => {
          const minion = makeBot(`godmin${i}`, s.kind, s.x, BOT_SPAWN_Y + 80, false);
  ...
  function initMultiplayer() {
    ...
    GameMP.init({
      ...
      subroom: matchMode === "trio" ? "showdown-trio" : matchMode === "duo" ? "showdown-duo" : "showdown-solo",
      getName: playerName,
      getState: () => ({
      ...
      onPeers(peers, count) {
        ...
        if (el) {
          if (isTeamMode() && peers.length > 0) {
            el.textContent = `👥 Team: ${peers[0].name || "Friend"}`;
          } else {
  ...
  function startWorld() {
    ...
    if (window.GameMP) GameMP.stop();
    enterFullscreen();
    showToast("🌍 Brawl Stars World — walk to the Merge Machine!");
    requestAnimationFrame(loop);
  }
